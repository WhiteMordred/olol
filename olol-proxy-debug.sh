...
HOST="0.0.0.0"
PORT=8000
SERVER_LIST="10.0.9.245:50051,10.0.9.248:50051,10.0.9.249:50051"
RPC_SERVER_LIST="10.0.9.245:50052,10.0.9.248:50052,10.0.9.249:50052"
LOG_DIR="./logs"
...
run_direct() {
  ...
  $PYTHON_BIN -c "from olol.proxy import run_proxy; run_proxy(host='$HOST', port=$PORT, server_addresses=['$SERVER_LIST'], enable_distributed=True, rpc_servers=['$RPC_SERVER_LIST'], debug=True, verbose=True)"
}

# Lancer le proxy avec l'entrée olol-proxy en mode debug
run_debug() {
  echo "[DEBUG] Lancer le proxy en mode debug"
  PYTHONPATH=$(pwd) FLASK_APP=olol.proxy FLASK_DEBUG=1 $PYTHON_BIN -m olol.proxy --host $HOST --port $PORT --servers $SERVER_LIST --rpc-servers $RPC_SERVER_LIST --distributed --verbose --debug
}

# Lancer le proxy avec un redirecteur de sortie pour capturer tous les logs
run_logged() {
  echo "[LOGGED] Lancer le proxy avec logs détaillés"
  PYTHONPATH=$(pwd) PYTHONUNBUFFERED=1 $PYTHON_BIN -m olol.proxy --host $HOST --port $PORT --servers $SERVER_LIST --rpc-servers $RPC_SERVER_LIST --distributed --verbose --debug > $LOG_DIR/proxy-debug.log 2>&1
}
